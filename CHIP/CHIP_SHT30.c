...
uint8_t CHIP_SHT30_request()
{
  ...
  if (hsht32Tran->transStageState == SHT30_TRANS_STATE_IDLE)
  {
    ...
    hsht32Tran->tickstart = HDL_G4_CPU_Time_GetTick(); /* 记录命令发送的时刻 */
...
void CHIP_SHT30_handler()
{
  ...
  {
    if ((HDL_G4_CPU_Time_GetTick() - hsht32Tran->tickstart) > SHT30_TRANS_TIMEOUT)
    {
    ...
    if ((HDL_G4_CPU_Time_GetTick() - hsht32Tran->tickstart) > SHT30_CONVERT_PERIOD)
    {
      uint16_t buff[6];
      uint16_t temp, humi;
