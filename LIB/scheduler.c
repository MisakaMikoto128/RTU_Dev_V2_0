...
void scheduler_handler()
{
    ...
    sc_list_foreach(gSchedulerList, it)
    {
        ...
        if (HDL_G4_CPU_Time_GetTick() >= (task->register_tick + task->delay_before_first_exe))
        {
            // 执行次数控制
            if (task->_exe_cnt < task->exe_times)
            {
                ...
                // 这里需要保证一定的实时性
                if ((HDL_G4_CPU_Time_GetTick() - task->last_exe_tick) >= task->period)
                {
                    bool ret = false;
                    ret = Functional_execute(&task->fun);
                    if (ret == true)
                    {
                        uint32_t current_tick = HDL_G4_CPU_Time_GetTick();
                        ...
                        if (task->_exe_tick_error > 0 && task->_exe_tick_error < task->period*2)
                        {
                            // TODO:这里的判断存在一定的问题
                            task->last_exe_tick -= task->_exe_tick_error;
                        }
                    ...
                    {
                        // 1ms补充轮询一次
                        task->last_exe_tick = HDL_G4_CPU_Time_GetTick() - task->period - 1;
                    }
                }
            }
        }
...
bool scheduler_register(SchedulerTask_t *task)
{
    ...
    if (task != NULL)
    {
        task->register_tick = HDL_G4_CPU_Time_GetTick();
...
bool period_query(uint8_t period_id, uint32_t period)
{
    ...
    // 这里一定是>=，如果是 > ，那么在1 cpu tick间隔的时候时间上是2cpu tick执行一次。
    // 这里不允许period为0，不然就会失去调度作用。
    if ((HDL_G4_CPU_Time_GetTick() - period_last_exe_tick_table[period_id]) >= period)
    {
        period_last_exe_tick_table[period_id] = HDL_G4_CPU_Time_GetTick();
        ret = true;
    }
    return ret;
}
