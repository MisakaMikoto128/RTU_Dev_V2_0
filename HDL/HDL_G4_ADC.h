#ifndef __HDL_G4_ADC__
#define __HDL_G4_ADC__
...
/* 在while(1)循环中
HDL_G4_ADC_Vaule(&ConvertValue.ADC1_Value,&ConvertValue.ADC2_Value);
*/

void HDL_G4_ADC_Init(void);//配置需要的功能并初始化,函数放在MX_初始化函数后面，其中MX_DMA_Init必须在MX_ADC1_Init之前

void HDL_G4_ADC_Enable(void);//启动双重ADC，定时器触发转换，DMA运输,函数放在HDL_G4_ADC_Init后面

void HDL_G4_ADC_Vaule(uint16_t *adc1_Value,uint16_t *adc2_Value);//计算ADC1的采集的数据和ADC2采集的数据，函数放在循环中
